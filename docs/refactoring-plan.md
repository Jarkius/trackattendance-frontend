...
- Prepare the codebase for automated testing (unit + integration) and future feature work such as offline queueing or alternative input devices.
...
2. **Telemetry and logging pipeline**
   - Introduce structured logging (e.g., JSON logs) from the Python side and surface key events (scan success/failure, exports) for future monitoring or audit requirements.
